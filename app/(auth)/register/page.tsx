...
export default function RegisterPage() {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      // Step 3: Send token to backend (auto-registers if new user)
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-6">
      <h1 className="text-base font-semibold text-text-primary">Create an account</h1>
      <p className="mt-1 text-sm text-text-tertiary">Set up your Atlas developer workspace.</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          {errors.full_name && <p className="text-xs text-remove">{errors.full_name.message}</p>}
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Input id="org_id" placeholder="org_xxxx" {...register("org_id")} />
          {errors.org_id && <p className="text-xs text-remove">{errors.org_id.message}</p>}
        </div>
        ...
        <Button type="submit" variant="signal" disabled={registerMutation.isPending} className="mt-1">
